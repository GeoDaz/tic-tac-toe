...
def create_grid(event, window, string_var, grid_msg, grid_cb):
    ...
    others_btn['restart'] = Button(
        window, text="Restart", width=10, command=lambda: restart(nb_cases, window, string_var))
    others_btn['restart'].grid(row=len(btn_dict)+2, column=0)
    others_btn['quit'] = Button(
        window, text="Quit", width=10, command=window.quit)
...
def define_grid(window, string_var):
    ...
    grid_cb.bind('<<ComboboxSelected>>', lambda e: create_grid(
        e, window, string_var, grid_msg, grid_cb))
...
def main():
    ...
    global frames_list
    frames_list = {}
    circle = PhotoImage(file="images/circle.png").zoom(2).subsample(20)
    cross = PhotoImage(file="images/cross.png").zoom(2).subsample(20)
